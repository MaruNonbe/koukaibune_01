...
(function () {
  ...
  // z: カメラからの距離。実際のモデルを解析したところ、拡大率5倍を
  //    かけると船の全長が約75mという非常に大きなサイズになるため、
  //    近い距離(-15)だと画面いっぱいに表示されてしまう。
  //    起動直後に少し離れた位置から見えるよう、距離を離した。
  //    もっと離す/近づけたい場合は、この値をさらに変更してください
  //    （マイナス方向に大きくするほど遠くなります）。
  const DEFAULT_STATE = {
    x: 0,
    y: -0.5,
    z: -28,
  ...
  const SETTINGS = {
    ...
    buttonMoveStep: 0.6,     // 十字キー：1フレームあたりの移動量（0.9→0.6に減速。まだ速い/遅い場合はここを調整）
